import java.util.Objects;

public class Voiture{
	
	private String numPlaque; // de type 1-AAA-999
	private String nomDuProprietaire;



	public Voiture(String numPlaque, String nomDuProprietaire){
		if(numPlaque==null)
			throw new IllegalArgumentException();
		if(nomDuProprietaire==null)
			throw new IllegalArgumentException();
		if(numPlaque.length()!=7)
			throw new IllegalArgumentException("plaque pas du type 1AAA999");
		this.numPlaque = numPlaque.toUpperCase();
		if(numPlaque.charAt(0)!='1')
			throw new IllegalArgumentException("plaque pas du type 1AAA999");
		for(int i=1; i<4; i++){
			if(this.numPlaque.charAt(i)<'A'||this.numPlaque.charAt(i)>'Z')
				throw new IllegalArgumentException("plaque pas du type 1AAA999");
		}
		for(int i=4; i<7; i++){
			if(this.numPlaque.charAt(i)<'0'||this.numPlaque.charAt(i)>'9')
				throw new IllegalArgumentException("plaque pas du type 1AAA999");
		}
		this.nomDuProprietaire = nomDuProprietaire; 
	}


	public String getNumPlaque() {
		return numPlaque;
	}


	public String getNomDuProprietaire() {
		return nomDuProprietaire;
	}

	@Override
	public String toString() {
		return "" + numPlaque + "("
				+ nomDuProprietaire + ")";
	}


	// ex a
	//	public int hashCode() {
//     return numPlaque.hashCode();
// }

	// ex b

	// public int hashCode() {
//     int resultat = 0;
//     for(int i = 0 ; i < 7 ; i++){
//         resultat +=(int) this.numPlaque.charAt(i);
//     }
//     return resultat;
// }




	//ex c

	@Override
	// public int hashCode() {
//     return Objects.hash(numPlaque);
// }


	//ex d - Idéal
	public int hashCode(){
		// Lettres de la plaque → indices dans l’alphabet
		int letters = (numPlaque.charAt(1) - 'A') * 26 * 26
		            + (numPlaque.charAt(2)) - 'A' * 26
					+ (numPlaque.charAt(3)) - 'A';
		int chiffres = Integer.parseInt((numPlaque.substring(4,7)));
		return letters * 1000 + chiffres;
	}
	
	
	
}

